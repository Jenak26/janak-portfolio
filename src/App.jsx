...
function Portfolio() {
  ...
  const archive = labProjects.filter(p => !p.featured && (filter === 'All' || p.category === filter));
  return <div id="home" className={`site-shell${paused ? ' motion-paused' : ''}`}>
    <a className="skip-link" href="#main">Skip to content</a>
    <header className="site-header"><a href="#home" className="wordmark" aria-label="Janak Kabra, home">j<span className="wordmark-star">✳</span>k<span className="wordmark-dot">.</span></a><span className="header-note mono">INDEPENDENT MIND.<br />INFINITE POSSIBILITIES.</span><nav aria-label="Main navigation"><a href="#work">Work <sup>04</sup></a><a href="#about">The human</a><a href="#contact" className="nav-contact">Let’s talk <Arrow /></a></nav></header>
    <main id="main">
      <section className="hero" aria-labelledby="hero-title">
        ...
        <div className="hero-composition"><h1 id="hero-title"><span>Serious code.</span><span className="second-line">Curious <em>mind.</em><span className="title-asterisk" aria-hidden="true">✳</span></span></h1>
          ...
          <a href="#work" className="work-orb" aria-label="Explore selected work"><span>EXPLORE<br />THE WORK</span><span aria-hidden="true">↓</span></a>
        </div>
      ...
      <section className="work-section section-padding" id="work"><div className="section-kicker mono"><span>01 / SELECTED WORK</span><span>IDEAS, MADE REAL.</span></div><div className="section-heading reveal"><h2>Out of my head.<br /><em>Into the world.</em></h2><p>A few rabbit holes worth going down.<br />From making algorithms visible to<br />teaching machines to order dinner.</p></div><div className="project-grid">{featured.map((p, i) => <ProjectCard key={p.title} project={p} index={i} paused={paused} />)}</div>
      <div className="archive"><div className="archive-heading"><h3>More from the lab <span>({labProjects.length - featured.length})</span></h3><div className="archive-filters" role="group" aria-label="Filter more projects">{['All', 'Systems', 'AI', 'Quant', 'Automation'].map(f => <button key={f} aria-pressed={filter === f} onClick={() => setFilter(f)}>{f}</button>)}</div></div><div className="archive-list">{archive.map(p => <details key={p.title}><summary><span className="archive-category mono">{p.category}</span><span className="archive-title">{p.title}</span><span className="archive-stack mono">{p.tech.slice(0, 2).join(' / ')}</span><span className="details-plus" aria-hidden="true">+</span></summary><div className="archive-content"><p>{p.description}</p><External href={p.github}>Explore repository <Arrow /></External>{p.live && <External href={p.live}>Visit live project <Arrow /></External>}</div></details>)}</div><External href="https://github.com/Jenak26" className="github-all">The source of it all. Find me on GitHub <Arrow /></External></div>
      </section>
...
export default function App() {
  return <BrowserRouter><Routes><Route path="/" element={<Portfolio />} /><Route path="*" element={<NotFound />} /></Routes></BrowserRouter>;
}
