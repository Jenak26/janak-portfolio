...
export default function OffScript() {
  ...
  return <div className={`offscript${paused ? ' motion-paused' : ''}`} id="os-top">
    ...
    <main id="os-main">
      ...
      <section className="os-work" id="os-work"><div className="os-section-label"><span>01 / PROJECT INDEX</span><span>SEVEN DIFFERENT RABBIT HOLES.</span></div><div className="os-work-heading"><h2>Proof of<br /><span>curiosity.</span></h2><p>Systems that hold up.<br />Experiences that stand out.<br />Explore what I’ve been building.</p></div><div className="os-filters" role="group" aria-label="Filter projects">{['Everything', 'Interactive', 'Systems', 'AI', 'Quant'].map(c => <button key={c} aria-pressed={category === c} onClick={() => setCategory(c)}>{c}{c === 'Everything' && <span>{String(labProjects.length).padStart(2, '0')}</span>}</button>)}<span aria-live="polite">{String(visible.length).padStart(2, '0')} PROJECTS</span></div>
